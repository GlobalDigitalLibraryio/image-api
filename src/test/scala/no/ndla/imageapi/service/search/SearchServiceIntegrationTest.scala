...
class SearchServiceIntegrationTest extends UnitSuite with TestEnvironment {
  ...
  val image4 = ImageMetaInformation(Some(4), List(ImageTitle("Hulken er ute og lukter på blomstene", LanguageTag("und"))), Seq(), smallImage.fileName, smallImage.size, smallImage.contentType, byNcSa, Seq(), Seq(), "ndla124", updated)
