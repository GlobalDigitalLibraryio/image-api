...
class SearchServiceIntegrationTest extends UnitSuite with TestEnvironment {
  ...
  val image1 = ImageMetaInformation(Some(1), None, List(ImageTitle("Batmen er på vift med en bil", nb)), List(ImageAltText("Bilde av en bil flaggermusmann som vifter med vingene bil.", nb)), largeImage.fileName, largeImage.size, largeImage.contentType, byNcSa, List(ImageTag(List("fugl"), nb)), List(), "ndla124", updated, Some(StorageService.CLOUDINARY))
  val image2 = ImageMetaInformation(Some(2), None, List(ImageTitle("Pingvinen er ute og går", nb)), List(ImageAltText("Bilde av en en pingvin som vagger borover en gate.", nb)), largeImage.fileName, largeImage.size, largeImage.contentType, publicDomain, List(ImageTag(List("fugl"), nb)), List(), "ndla124", updated, Some(StorageService.CLOUDINARY))
  val image3 = ImageMetaInformation(Some(3), None, List(ImageTitle("Donald Duck kjører bil", nb)), List(ImageAltText("Bilde av en en and som kjører en rød bil.", nb)), smallImage.fileName, smallImage.size, smallImage.contentType, byNcSa, List(ImageTag(List("and"), nb)), List(), "ndla124", updated, Some(StorageService.CLOUDINARY))
  val image4 = ImageMetaInformation(Some(4), None, List(ImageTitle("Hulken er ute og lukter på blomstene", LanguageTag("und"))), Seq(), smallImage.fileName, smallImage.size, smallImage.contentType, byNcSa, Seq(), Seq(), "ndla124", updated, Some(StorageService.CLOUDINARY))
  val image5 = ImageMetaInformation(Some(5), None, List(ImageTitle("Dette er et urelatert bilde", LanguageTag("und")), ImageTitle("This is a unrelated photo", LanguageTag("en")), ImageTitle("Nynoreg", LanguageTag("nn"))), Seq(ImageAltText("urelatert alttext", LanguageTag("und")), ImageAltText("Nynoreg", LanguageTag("nn"))), smallImage.fileName, smallImage.size, smallImage.contentType, byNcSa.copy(agreementId = Some(1)), Seq(), Seq(), "ndla124", updated, Some(StorageService.CLOUDINARY))
